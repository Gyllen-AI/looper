...
export function stackDocument(stack: Stack, when: string): string {
  return [
    ...
    "",
    "Before adding one, read `STACK.md` inside the installed package, at",
    "`node_modules/looper/STACK.md`. It is looper's recommendation for a new",
    "service — the web framework, the database layer, the front end, one tool per",
    "job — and it judges nothing here. It exists so that adding a row is a choice",
    "between named options rather than whatever was reached for first.",
    "",
