...
export const CSHARP_RULES: readonly Rule[] = [
  {
    ...
    instead: [
      ...
      "a file a program wrote is exempt in full — mark it `// <auto-generated/>`, which is what .NET already looks for",
      "a licence header on the first line is exempt, and the one other exception is `looper:allow-secret` after code on the same line, which the commit gate checks on every commit and so cannot quietly stop being true",
    ],
    valve: { kind: "none" },
  },
  {
