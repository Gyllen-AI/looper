...
**Python, four rules so far.** A `.py` file is read with Python's own parser — no
...
the mutable default argument, and `PY-TRUTH:2`, `assert` used where `python -O`
will delete it.
[STACK.md](STACK.md) prescribes the rest of the Python stack and `docs/PLAN.md`
names the other three rules, none of them written yet. Nothing pretends
otherwise.
